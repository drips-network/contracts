...
abstract contract Pool {
    ...
    function _setDeltasFromNow(
        ...
        while (true) {
            ...
            // Find an existing receiver stream log to update
            uint256 updated = 0;
            while (updated < updates.length && updates.updates[updated].receiver != receiverAddr) {
                updated++;
            }
            // New receiver, will be pushed
            if (updated == updates.length) {
                updates.length++;
            }
            updates.updates[updated] = StreamUpdate({
                receiver: receiverAddr,
                amtPerSec: amtPerSecDelta > 0 ? uint128(amtPerSecDelta) : 0,
                endTime: amtPerSecDelta > 0 ? timeEnd : _currTimestamp()
            });
        }
    }
