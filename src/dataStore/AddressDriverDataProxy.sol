...
contract AddressDriverDataProxy is ERC2771Context, Managed {
    ...
    constructor(AddressDriver addressDriver_, DripsDataStore dripsDataStore_, Caller caller_)
    ...
    function setSplits(bytes32 splitsHash) public whenNotPaused {
        bytes memory data =
    ...
    /// the actual list must be stored in DripsDataStore.
    function emitAccountMetadata(bytes32 accountMetadataHash) public whenNotPaused {
        bytes memory data = abi.encodeCall(
